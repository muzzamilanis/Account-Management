...
pages = [
    ...
    ("Manage Parties", "CUSTOMERS"),
    ("Manage Agents", "AGENTS"),
    ("Sale Autos", "SALES"),
    ("Reports", "REPORTS")
]

for header, uppercase in pages:
    # Open tab item
    content = re.sub(
        rf'(?s)                    <!-- {uppercase} PAGE -->\s*<Grid>\s*<Grid\.Style>.*?</Grid\.Style>',
...
content = re.sub(
    r'(?s)                        </Grid>\s*</DockPanel>\s*</Grid>\s*</Grid>\s*</Border>\s*<!-- -- STATUS BAR -- -->',
    r'                        </Grid>\n                    </DockPanel>\n                        </Grid>\n                    </TabItem>\n        </TabControl>\n\n        <!-- -- STATUS BAR -- -->',
    content
)
